...
const STATUS_STYLES = {"live":"background:#dcfce7;color:#166534","delisted":"background:#fee2e2;color:#991b1b","paused":"background:#fef3c7;color:#92400e","tac":"background:#ede9fe;color:#5b21b6","never went live":"background:#f3f4f6;color:#6b7280"};
function StatusChip({status}) {
  const s=status?.toLowerCase()||"";
  const style=STATUS_STYLES[s]||"background:#f3f4f6;color:#4b5563";
  const styleObj = Object.fromEntries(style.split(";").map(p=>p.split(":").map(x=>x.trim())).filter(p=>p.length===2).map(([k,v])=>[k.replace(/-([a-z])/g,(_,l)=>l.toUpperCase()),v]));
  return <span style={{...styleObj,display:"inline-flex",padding:"0.15rem 0.5rem",borderRadius:"9999px",fontSize:"0.75rem",fontWeight:500}}>{status||"—"}</span>;
}
const norm = (s) => s?.trim().toLowerCase().replace(/\s+/g," ") || "";
export default function PropertyTable({ supplyData, revenueData, filters }) {
      ...
      <div style={{background:"white",borderRadius:"1rem",border:"1px solid #f3f4f6",boxShadow:"0 1px 3px rgba(0,0,0,0.04)",overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table className="w-full data-table" style={{whiteSpace:"nowrap"}}>
            ...
            <tbody>
              {properties.length===0 ?
                <tr><td colSpan={9} style={{textAlign:"center",color:"#9ca3af",padding:"3rem"}}>No properties found</td></tr> :
                properties.map((p,idx)=>{
                  ...
                  return (
                    <tr key={idx}>
                      ...
                      <td style={{textAlign:"center",fontWeight:500}}>{p.rooms||"—"}</td>
                      ...
                      <td style={{color:"#374151"}}>{rev.totalNights?rev.totalNights:"—"}</td>
                    </tr>
                  );
                })
              }
